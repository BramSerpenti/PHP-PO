...
//session start is nodig voor elke pagina boven aan, zodat de pagina het id van de gebruiker onthoud
session_start();
if (!isset($_SESSION["id"]) || $_SESSION["role"] != 2) {// zorgt er voor dat de pagina enkel beschikbaar zijn voor mensen met een account.
    header("Location: homepaginaphp.php");// stuurt iedereen terug naar de inlog pagina.
    exit;
}

$conn = new mysqli("localhost", "root", "", "po_webapp");//verbindt de server met de database
if ($conn->connect_error) {//checkt of de server goed is opgestart.
